...
/**
 * Displays a menu of items in the console and waits for the user to select one.
 *
 ...
 * @returns The chosen item, or null if the menu was cancelled.
 */
function chooseAsync(items, options) {
    ...
    if (selectedIndex < 0) {
        selectedIndex = 0;
        while (selectedIndex < count && items[selectedIndex].isSeparator) selectedIndex++;
    }
...
function printMenu(items, options, selectedIndex, scrollOffset) {
    ...
    let prefix = (options.border ? '|' : '');
    let suffix = (options.border ? ' |' : '');
    ...
    let scrollEnd = options.pageSize
    ...
    for (let i = scrollOffset; i < scrollEnd; i++) {
        if (items[i].separator) {
            ...
            let hotkey = items[i].hotkey || '*';
            let title = items[i].title || '';
            let label = (i === selectedIndex
                ? '[' + hotkey + ']' : ' ' + hotkey + ')');
